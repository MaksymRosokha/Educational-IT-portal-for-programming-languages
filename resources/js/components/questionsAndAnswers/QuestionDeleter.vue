<template>
  <div class="question-deleter">
    <form class="question-deleter__form" @submit.prevent="sendData">
      <p class="question-deleter__label">
        Ви впевнені, що бажаєте видалити запитання?
      </p>

      <input type="hidden" name="_token" :value="csrf">
      <form-button class="question-deleter__btn-submit">Видалити</form-button>
    </form>

    <success-or-fail-modal-window
        class="question-deleter__result-window result-window"
  ...
  methods: {
    sendData() {
      ...
      axios.post(this.link, formData)
          .then(response => {
            this.result.text = 'Питання успішно видалено';
          ...
          .catch(error => {
            if (error.response && error.response.data && error.response.data.errors) {
              ...
              this.result.text = 'Не вдалося видалити питання';
